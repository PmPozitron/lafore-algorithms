...
class Tree234 {
    ...
    public DataItem findMinimum(Node current) {
        if (current == null) {
            current = root;
        }
    ...
    private void sortingTraverse(Node current, List<Long> result) {

        if (current.getChild(0) != null) {
            sortingTraverse(current.getChild(0), result);
        }

        if (current.getItem(0) != null) {
            result.add(current.getItem(0).dData);
        }

        if (current.getChild(1) != null) {
            sortingTraverse(current.getChild(1), result);
        }
        if (current.getItem(1) != null) {
            result.add(current.getItem(1).dData);
        }
        if (current.getChild(2) != null) {
            sortingTraverse(current.getChild(2), result);
        }
        if (current.getItem(2) != null) {
            result.add(current.getItem(2).dData);
        }
        if (current.getChild(3) != null) {
            sortingTraverse(current.getChild(3), result);
        }
    }

    public void sort(long[] toBeSorted) {
        for (long value : toBeSorted) {
            this.insert(value);
        }

        ArrayList<Long> result = new ArrayList<Long>();
        sortingTraverse(root, result);
...
//        theTree.insert(35);
//        theTree.insert(15);
        long[] values = new long[]{12, 13, 59, 5, 57, 34, 89, 65, 28};
